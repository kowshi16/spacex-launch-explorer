...
export const getUniqueYears = (launches) => {
  const years = launches.map(launch => new Date(launch.date_local).getFullYear());
...
export const filterLaunches = (launches, filters) => {
  return launches.filter(launch => {
    ...
    const matchesFavorites = !filters.favoritesOnly || 
      filters.favorites.includes(launch.id);

    return matchesSearch && matchesYear && matchesSuccess && matchesFavorites;
  });
};

export const sortLaunchesByDate = (launches) => {
  return [...launches].sort((a, b) => new Date(b.date_local) - new Date(a.date_local));
};
